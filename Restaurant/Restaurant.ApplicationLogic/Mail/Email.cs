...
namespace Restaurant.ApplicationLogic.Mail
{
    public class Email : IEquatable<Email>
    {
        ...
        public Email(string email)
        {
            ...
            {
                throw new RestaurantServerException("Email cannot be empty", typeof(Email).FullName, "EmailOperation");
            }

            if (!Regex.Match(email, EMAIL_PATTERN).Success)
            {
                throw new RestaurantServerException("Invalid Email", typeof(Email).FullName, "EmailOperation");
            }
